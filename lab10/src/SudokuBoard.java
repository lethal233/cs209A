...
/*
Object that will hold a Sudoku Board within a 2-D array.
Squares that cannot be moved/change are kept track. Also allows
for randomly filling in blank positions/0 on the board with valid
numbers.
Note - This class allows users to use a pre-filled board based on the
...
public class SudokuBoard {
    ...
    */
    public void copySudokuBoard(SudokuBoard sb) {
        ...
        for (int i = 0; i < arr.length; i++) {
            System.arraycopy(arr[i], 0, this.board[i], 0, arr[i].length);
        }
    ...
    */
    ...
    /*
    Method used when manually inserting the board
    */
    public void insert(int row, int col, int value) {
        if (value > 0) {
            //Set the position as unchangable
            forbiddenMove[row][col] = true;

            //Set the value on board
            board[row][col] = value;

            //Remove value from number pool
            numberPool.get(col).remove(new Integer(board[row][col]));
        }
    }
...
//        SudokuBoard sdkBrd = new SudokuBoard(true);
